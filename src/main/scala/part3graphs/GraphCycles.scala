...
object GraphCycles extends App {
  ...
  // graph cycle deadlock: backpressure reason
  ...
  /**
    ...
    * - output will emit an INIFNITE FIBONNACI SEQUENCE based off those 2 numbers
    * 1, 2, 3, 5, 8, ...
    *
    * Hint: use ZipWith and cycles, MergePreferred
    */

  val fibonnaciGenerator = GraphDSL.create() { implicit builder =>
    import GraphDSL.Implicits._
  ...
  val fiboGraph = RunnableGraph.fromGraph(
    GraphDSL.create() { implicit builder =>
      ...
      val fibo = builder.add(fibonnaciGenerator)
